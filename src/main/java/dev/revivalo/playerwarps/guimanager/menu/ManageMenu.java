...
public class ManageMenu implements Menu {
    ...
    public void open(Player player) {
        ...
        if (!Config.ENABLE_CATEGORIES.asBoolean()) gui.setItem(Config.SET_CATEGORY_POSITION.asInt(), ItemBuilder.from(warp.getCategory() == null ? new ItemStack(Material.WHITE_BANNER) : warp.getCategory().getItem()).name(Component.text(Lang.CHANGE_TYPE.asColoredString())).setLore(Lang.CHANGE_TYPE_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> new ChangeTypeMenu(warp).open(player))); //openChangeTypeMenu(player, warp)));
        ...
        gui.setItem(Config.CHANGE_DESCRIPTION_POSITION.asInt(), ItemBuilder.from(Material.WRITABLE_BOOK).name(Component.text(Lang.CHANGE_DISPLAY_NAME.asColoredString())).setLore(Lang.CHANGE_DISPLAY_NAME_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> PlayerWarpsPlugin.getWarpHandler().markPlayerForChatInput(player, warp, WarpAction.CHANGE_DISPLAY_NAME)));
