...
public class ManageMenu implements Menu {
    ...
    public void open(Player player) {
        ...
        if (Hooks.isHookEnabled(Hooks.getVaultHook()) && Config.MAX_WARP_ADMISSION.asInteger() > 0) gui.setItem(Config.SET_PRICE_POSITION.asInteger(), ItemBuilder.from(Material.SUNFLOWER).setName(Lang.SET_PRICE.asColoredString().replace("%warp%", warp.getName())).setLore(Lang.SET_PRICE_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> PlayerWarpsPlugin.getWarpHandler().markPlayerForChatInput(player, warp, WarpAction.SET_ADMISSION)));
        if (Config.ENABLE_CATEGORIES.asBoolean()) gui.setItem(Config.SET_CATEGORY_POSITION.asInteger(), ItemBuilder.from(warp.getCategory() == null ? new ItemStack(Material.WHITE_BANNER) : warp.getCategory().getItem()).setName((Lang.CHANGE_TYPE.asColoredString())).setLore(Lang.CHANGE_TYPE_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> new ChangeTypeMenu(warp).open(player)));
        gui.setItem(Config.CHANGE_DISPLAY_NAME_POSITION.asInteger(), ItemBuilder.from(Material.WRITABLE_BOOK).setName(Lang.CHANGE_DISPLAY_NAME.asColoredString()).setLore(Lang.CHANGE_DISPLAY_NAME_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> PlayerWarpsPlugin.getWarpHandler().markPlayerForChatInput(player, warp, WarpAction.CHANGE_DISPLAY_NAME)));
        gui.setItem(Config.CHANGE_PREVIEW_ITEM_POSITION.asInteger(), ItemBuilder.from(Material.ITEM_FRAME).setName(Lang.CHANGE_ITEM.asColoredString()).setLore(Lang.CHANGE_ITEM_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> PlayerWarpsPlugin.getWarpHandler().markPlayerForChatInput(player, warp, WarpAction.SET_GUI_ITEM)));
        gui.setItem(Config.CHANGE_DESCRIPTION_POSITION.asInteger(), ItemBuilder.from(Material.NAME_TAG).setName(Lang.CHANGE_DESCRIPTION.asColoredString()).setLore(Lang.CHANGE_DESCRIPTION_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> PlayerWarpsPlugin.getWarpHandler().markPlayerForChatInput(player, warp, WarpAction.SET_DESCRIPTION)));
        gui.setItem(Config.CHANGE_ACCESSIBILITY_POSITION.asInteger(), ItemBuilder.from(Material.IRON_DOOR).setName(Lang.PWARP_ACCESSIBILITY.asColoredString()).setLore(Lang.PWARP_ACCESSIBILITY_LORE.asReplacedList(player, new HashMap<String, String>() {{
            put("%status%", warp.getStatus().getText());
        }})).asGuiItem(event -> new SetStatusMenu(warp).open(player))); //openSetStatusMenu(player, warp)));
        gui.setItem(Config.RENAME_WARP_POSITION.asInteger(), ItemBuilder.from(Material.OAK_SIGN).setName(Lang.RENAME_WARP.asColoredString()).setLore(Lang.RENAME_WARP_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> PlayerWarpsPlugin.getWarpHandler().markPlayerForChatInput(player, warp, WarpAction.RENAME)));
        gui.setItem(Config.REMOVE_WARP_POSITION.asInteger(), ItemBuilder.from(Material.BARRIER).setName(Lang.REMOVE_WARP.asColoredString()).setLore(Lang.REMOVE_WARP_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> new ConfirmationMenu(warp).open(player, WarpAction.REMOVE))); //openAcceptMenu(player, warp, WarpAction.REMOVE)));
        gui.setItem(Config.RELOCATE_WARP_POSITION.asInteger(), ItemBuilder.from(Material.WHITE_BANNER).setName(Lang.WARP_RELOCATION.asColoredString()).setLore(Lang.WARP_RELOCATION_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> new RelocateAction().preExecute(player, warp, null, MenuType.MANAGE_MENU, 1)));//PlayerWarpsPlugin.getWarpHandler().relocateWarp(player, warp)));
        gui.setItem(Config.CHANGE_OWNER_POSITION.asInteger(), ItemBuilder.from(Material.PLAYER_HEAD).setName(Lang.CHANGE_OWNER.asColoredString()).setLore(Lang.CHANGE_OWNER_LORE.asReplacedList(player, Collections.emptyMap())).asGuiItem(event -> PlayerWarpsPlugin.getWarpHandler().markPlayerForChatInput(player, warp, WarpAction.CHANGE_OWNERSHIP)));
