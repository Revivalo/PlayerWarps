...
public class ReviewMenu implements Menu {
    ...
    public void open(Player player) {
        ...
        gui.setItem(31, ItemBuilder.from(ItemUtils.getItem(Config.BACK_ITEM.asString())).name(Component.text(Lang.BACK_NAME.asColoredString())).asGuiItem(event -> new WarpsMenu(MenuType.DEFAULT_LIST_MENU, ((int) UserManager.getUsersTemp(player.getUniqueId())[0])).open(player, warp.getCategory() == null ? "all" : warp.getCategory().getType(), SortingUtils.SortType.LATEST)));
        gui.open(player);
    }
}
