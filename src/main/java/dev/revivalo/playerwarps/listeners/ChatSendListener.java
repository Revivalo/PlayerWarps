        ...
        PlayerWarpsPlugin.get().runSync(() -> {
            switch (warpAction){
                case CHANGE_DISPLAY_NAME:
                    new ChangeDisplayNameAction().preExecute(editor, warp, message, MenuType.SET_UP_MENU);
                    //PlayerWarpsPlugin.getWarpHandler().changeDisplayName(editor, warp, message);
                    break;
                case CHANGE_OWNERSHIP:
                    new TransferOwnershipAction().preExecute(editor, warp, Bukkit.getPlayer(message), MenuType.SET_UP_MENU);
                    //PlayerWarpsPlugin.getWarpHandler().transferOwnership(editor, Bukkit.getPlayer(message), warp, shouldReOpenMenu);
                    break;
                case RENAME:
                    new RenameAction().preExecute(editor, warp, message, MenuType.SET_UP_MENU);
                    //PlayerWarpsPlugin.getWarpHandler().rename(editor, warp, message, shouldReOpenMenu);
                    break;
                case SET_ADMISSION:
                    new SetAdmissionAction().preExecute(editor, warp, message, MenuType.SET_UP_MENU);
                    //PlayerWarpsPlugin.getWarpHandler().setAdmission(editor, warp, message, shouldReOpenMenu);
                    break;
                case SET_GUI_ITEM:
                    new SetPreviewItemAction().preExecute(editor, warp, message, MenuType.SET_UP_MENU);
                    //PlayerWarpsPlugin.getWarpHandler().setItem(editor, warp, message, shouldReOpenMenu);
                    break;
                case SET_DESCRIPTION:
                    new SetDescriptionAction().preExecute(editor, warp, message, MenuType.SET_UP_MENU);
